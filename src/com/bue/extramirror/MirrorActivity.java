...
@SuppressLint("NewApi")
public class MirrorActivity extends FragmentActivity implements SettingsDialogFragment.SettingsDialogListener, ExtraMirrorSharedPreferences{
...
//		}
		...
		speedometerIndicatorMode=sharedSettings.getInt(PREFS_SPEEDINDICATOR, 0);
		backPressed=false;
        ...
        if((metrics.heightPixels==320 && metrics.widthPixels==240)||(metrics.heightPixels==240 && metrics.widthPixels==320)){
        	smallerDimension=(metrics.widthPixels/2<metrics.heightPixels)?metrics.widthPixels/2-20:metrics.heightPixels-20;
        }else if((metrics.heightPixels==480 && metrics.widthPixels==800)||(metrics.heightPixels==800 && metrics.widthPixels==480)){
        	smallerDimension=((2*metrics.widthPixels)/3<metrics.heightPixels)?(2*metrics.widthPixels)/3-20:metrics.heightPixels-20;
        }else{
        	smallerDimension=(metrics.widthPixels<metrics.heightPixels)?metrics.widthPixels-20:metrics.heightPixels-20;
        }      
        smallerDimension=15*smallerDimension/16;
        if(measureUnit==0)
        	analogSpeedMeter=new AnalogSpeedMeterView(this, smallerDimension, "Km/h", speedometerIndicatorMode);
        else
        	analogSpeedMeter=new AnalogSpeedMeterView(this, smallerDimension, "mph", speedometerIndicatorMode);
        lpAnalogSpeedMeter=new LayoutParams((int)smallerDimension, (int)smallerDimension);
		...
		//TODO: Notification		boolean	toggleEngine=false;//It is used to toggle engine button if it was active before Notification Button used
		...
		if(savedInstanceState!=null){
			...
			elapsedTimeTextView.setText(clock.convertTime(drivingTime));
			idleTimeTextView.setText(clock.convertTime(idleTime));
			startEngine = extras.getBoolean(ENGINE_STATE);			
		...
		final Runnable drivingTimeRunnable = new Runnable() {
			public void run() {
				...
				//TODO: open on debug Log.d("TimerRunning", "Driving for... " + drivingTime);
				Clock clock = new Clock();
				elapsedTimeTextView.setText(clock.convertTime(drivingTime));
				distance += speedms;
				updateDistance();
		...
		final Runnable idleTimeRunnable = new Runnable() {
			public void run() {
				idleTime += 1000;
				//TODO: open on debug Log.d("TimerIdle", "Idle for... " + idleTime);
				Clock clock = new Clock();
				idleTimeTextView.setText(clock.convertTime(idleTime));
				runnableHandlers.postDelayed(this, 1000);
			}
		...
		final Runnable watch = new Runnable() {
			public void run() {
				...
				//TODO: open on debug Log.d("TimerWatch", "Watch for... "+now.format("%k:%M|%d-%m-%Y"));
				runnableHandlers.postDelayed(this, 1000);
			}
		};
...
//				randomGenerator.setSeed((long)(location.getAltitude()*100));
				...
				//TODO: open on debug Log.d("Locatiob: ", latitude+":"+longitude);
				...
				if (startEngine){
					...
					} else if((prvSpeed == 0 || wasEngineInactive) && speed > 0) {
		...
		if (mPreview == null) {
			initializeCameraPreview(sharedSettings.getInt(PREFS_ACTIVE_CAMERA, 0));
		}
		...
		optionsMenuImageButton.setOnTouchListener(new View.OnTouchListener(){
				public boolean onTouch(View arg0, MotionEvent event) {
					optionsMenuImageButton.setImageResource(R.drawable.menu_moreoverflow_normal_holo_light_rotate_90_pressed);
					...
					case MotionEvent.ACTION_UP:
						if(actionBar.isShowing())
							...
							actionBar.show();	
						optionsMenuImageButton.setImageResource(R.drawable.menu_moreoverflow_normal_holo_light_rotate_90);					
						break;
					}
				return true;
			}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
	        case R.id.menu_settings:
	            ...
	            return true;
	        case R.id.menu_exit:
	        	//getWindow().setAttributes(systemParameters);//Set System Screen Settings
	        	backPressed=true;
...
//		intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);//***Change Here***		   		
	...
	public void initializeCameraPreview(int lastActiveCamera){		
		...
		if (Camera.getNumberOfCameras() == 0) {
			...
			if(lastActiveCamera==MIRROR_CAMERA || lastActiveCamera==NO_CAMERA){
				cameraJustChanged=true;
...
//				mCamera=null;
	 ...
	 * (non-Javadoc)
	 * @see com.bue.extramirror.SettingsDialogFragment.SettingsDialogListener#onDialogPositiveClick(android.support.v4.app.DialogFragment)
	 */

	public void onDialogPositiveClick(DialogFragment dialog) {
		//TODO: open on debug Log.d("onDialogPositiveClick","onDialogPositiveClick");
		...
		switch(measureUnit){
			...
			case 1: 
				...
				editor.commit();
				break;
			default:editor.putInt(PREFS_MEASURE_UNIT, 0);
			editor.commit();
			break;
		}
